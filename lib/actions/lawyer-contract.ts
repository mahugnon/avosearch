...
export type LawyerContractUpdateResult = { ok: true } | { error: string };

async function assertLawyerMissionAccess(contractId: string, missionId?: string) {
...
export async function updateLawyerContractAction(input: {
  contractId: string;
  missionId?: string;
  extractedText: string;
  draftAnswers?: Record<string, string>;
}): Promise<LawyerContractUpdateResult> {
  const access = await assertLawyerMissionAccess(input.contractId, input.missionId);
  if (!("ok" in access)) {
...
export async function loadLawyerContractHighlight(contract: {
  template: {
    fileKey: string | null;
