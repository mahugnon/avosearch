...
function isMissionsArea(pathname: string): boolean {
  return (
    pathname === "/lawyer" ||
    pathname.startsWith("/lawyer/missions") ||
    pathname.startsWith("/lawyer/contracts")
  );
}

export function LawyerSidebar({ actionCount }: LawyerSidebarProps) {
  ...
  const items: NavItem[] = [
    ...
    },
    {
      ...
      labelKey: "profile",
      icon: UserCircle,
      match: (p) => p.startsWith("/lawyer/profile"),
    },
  ];
