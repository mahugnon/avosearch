...
import { registerBarristerAction } from "@/lib/actions/auth";
...
export function RegisterBarristerForm() {
  const t = useTranslations("auth");
  const [state, formAction, pending] = useActionState(registerBarristerAction, undefined);

  return (
    <Card className="surface-elevated border-0 shadow-none">
      <CardHeader>
        <CardTitle>{t("barristerRegisterTitle")}</CardTitle>
        <CardDescription>{t("barristerRegisterDescription")}</CardDescription>
      </CardHeader>
      <form action={formAction}>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              name="name"
              autoComplete="name"
              placeholder={t("barristerNamePlaceholder")}
              required
            />
          </div>
          <div className="space-y-2">
        ...
        <CardFooter className="mt-6 flex flex-col gap-3">
          <Button type="submit" className="w-full" disabled={pending}>
            {pending ? t("creatingAccount") : t("createBarristerAccount")}
          </Button>
